...
| `47_connector_athena.js` | `AthenaConnector` (registered as `ConnectorRegistry['athena']`). Implemented: `getConfigSchema`, `testConnection`, `setupDatabase` (CREATE DATABASE + 18 CREATE EXTERNAL TABLE, idempotent), plus helpers `athenaApiCall`, `athenaQuery` (StartQueryExecution + poll to completion), `athenaGetResults` (paginated GetQueryResults), `s3PutObject` (signed PUT, `s3ObjectUrl` / `s3EncodeSegment` / `s3FormatError`), `athenaCreateTableDDL`, `athenaTableDataKey`, `athenaTableLocation`, `athenaAssertIdentifier`, `importAllTables` (20 steps: connect + 18 SELECTs + completion) with its coercion helpers `athenaCoerceRecord` / `athenaCoerceTable` / `athenaUnfetchedTables`, and `exportAllTables` (20 steps: connect + 18 uploads + completion) with its write helpers `athenaBuildTableCSV` / `athenaCsvField` / `athenaSanitiseValue` / `athenaExportTable`. Every connector method is now implemented; nothing is stubbed. Both transfer directions share one retry policy in `athenaRunWithRetry` (3 attempts, 2 s then 5 s, constants `ATHENA_TRANSFER_ATTEMPTS` / `ATHENA_TRANSFER_RETRY_WAIT_MS`) -- the retry unit is the whole per-table sequence, and each attempt re-runs it from the start because `athenaQuery` must mint a fresh `ClientRequestToken` per call. Coercion and CSV building sit outside the retry: both are deterministic, so a second pass would fail identically. `ATHENA_TABLES` is the authoritative 18-table list published to the cloud -- explicitly **not** `Object.keys(SCHEMA)`, which has 22. `importAllTables` returns `{ ok, data, warnings, failedTables }` and its `data` omits the four SCHEMA tables outside `ATHENA_TABLES`, so callers must merge it into state, never assign over state, and must apply nothing unless `ok`. `exportAllTables` returns `{ ok, failedTables, rowCounts, errors }` and is all-or-nothing -- on `ok: false` the whole export must be re-run, which is always safe because each table is DROP + CREATE. The Athena write path has **its own CSV writer** and deliberately does not use `tableToCSV()` (`40_storage.js`), which must not be changed to suit it: `OpenCSVSerde` cannot read a newline inside a value and treats `\` as an escape, so `athenaSanitiseValue` collapses newlines and doubles backslashes -- lossy fixes that would corrupt the V1 ZIP and uploader exports if applied to the shared function. All SQL must be fully qualified -- `QueryExecutionContext` is never sent |
