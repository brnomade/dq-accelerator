...
| `47_connector_athena.js` | `AthenaConnector` (registered as `ConnectorRegistry['athena']`). Implemented: `getConfigSchema`, `testConnection`, `setupDatabase` (CREATE DATABASE + 18 CREATE EXTERNAL TABLE, idempotent), plus helpers `athenaApiCall`, `athenaQuery` (StartQueryExecution + poll to completion), `athenaGetResults` (paginated GetQueryResults), `s3PutObject` (signed PUT, `s3ObjectUrl` / `s3EncodeSegment` / `s3FormatError`), `athenaCreateTableDDL`, `athenaTableDataKey`, `athenaTableLocation`, `athenaAssertIdentifier`, plus `importAllTables` (20 steps: connect + 18 SELECTs + completion) with its coercion helpers `athenaCoerceRecord` / `athenaCoerceTable` / `athenaUnfetchedTables`. `ATHENA_TABLES` is the authoritative 18-table list published to the cloud -- explicitly **not** `Object.keys(SCHEMA)`, which has 22. Stubbed pending plan task 3.7: `exportAllTables`. `importAllTables` returns `{ ok, data, warnings, failedTables }` and its `data` omits the four SCHEMA tables outside `ATHENA_TABLES`, so callers must merge it into state, never assign over state, and must apply nothing unless `ok`. All SQL must be fully qualified -- `QueryExecutionContext` is never sent |
