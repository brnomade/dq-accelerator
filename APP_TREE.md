...
| `47_connector_athena.js` | `AthenaConnector` (registered as `ConnectorRegistry['athena']`). Implemented: `getConfigSchema`, `testConnection`, `setupDatabase` (CREATE DATABASE + 18 CREATE EXTERNAL TABLE, idempotent), plus helpers `athenaApiCall`, `athenaQuery` (StartQueryExecution + poll to completion), `athenaGetResults` (paginated GetQueryResults), `s3PutObject` (signed PUT, `s3ObjectUrl` / `s3EncodeSegment` / `s3FormatError`), `athenaCreateTableDDL`, `athenaTableDataKey`, `athenaTableLocation`, `athenaAssertIdentifier`. `ATHENA_TABLES` is the authoritative 18-table list published to the cloud -- explicitly **not** `Object.keys(SCHEMA)`, which has 22. Stubbed pending plan tasks 3.6-3.7: `importAllTables`, `exportAllTables`. All SQL must be fully qualified -- `QueryExecutionContext` is never sent |
