...
The application implements 22 tables across 5 logical groups, based on the physical data model for the MoJ Data Quality Store (`mojdm_dataquality_store`).

### Group 1 — Data Quality Elements
`critical_data_set`, `critical_data_element`, `data_quality_rule`, `data_quality_rule_allocation`, `cde_criticality`, `shortlist_group`, `cde_shortlist_tag`

### Group 2 — Ownership Hierarchy
`executive_agency`, `directorate`, `stewardship`, `data_patron`, `data_owner`, `data_steward`
...
- **Delta sync** — `DELTA_TABLES` references schema-defined table names to determine which tables are included in the delta snapshot and export
...
| `moj_dq_master_v1` | Master designation `{ stewardId }` — the steward ID of the designated master user |
...
| `moj_dq_groups_v1` | Per-group nav collapsed/expanded state |
| `moj_dq_assistant_v1` | DQ Assistant session state (rule intent, CDE selection, proposals) |
...
The master user is identified by a `moj_dq_master_v1` localStorage entry containing `{ stewardId }`. This is set via the Settings panel by any user whose current identity matches the stored ID. The designation persists independently of the application data, so it survives a master import (data reset).

On startup, the app performs a one-time migration: if a legacy sentinel stewardship record (`critical_data_set_id = 0`) is found in localStorage data, it is converted to the `moj_dq_master_v1` key and removed from the stewardship table automatically.
...
All 22 tables are included in the delta snapshot and export. Changes to any table — including reference data, org hierarchy, and weights — are detected and surfaced in the delta. In practice, steward copies only accumulate changes in the tables accessible to their role (the Ownership Hierarchy, Weights, and Core Settings sidebar groups are hidden for non-master users), but the mechanism does not restrict coverage by table.
...
| Master | Identity matches the steward ID in `moj_dq_master_v1` (set via Settings) | All steward capabilities plus: access all sidebar groups, export master, import delta, designate master |
...
- **Top items** (always visible) — Dashboard, DQ Assistant
- **Data Quality Elements** — Data and Stewardship, RAG Simulator, Rule Generator, Rules Explorer
- **Database Actions** — Export, Import, Data Browser (master only), Profiling
- **Ownership Hierarchy** (master only) — Organisation (chart), Directorate, Data Patron, Data Owner, Data Steward
- **Weights & Thresholds** (master only) — Criticality Group Weight, Quality Dimension Weight
- **Core Settings** (master only, read-only) — Executive Agency Type, Steward Role Type, Quality Dimension, Criticality Group, Criticality Level

The Ownership Hierarchy, Weights & Thresholds, and Core Settings groups are hidden entirely for non-master users. The sidebar is collapsible. Record counts are shown on each table item.
...
- 22 SharePoint Lists provisioned (one per table)
