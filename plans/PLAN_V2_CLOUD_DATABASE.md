...
**Date:** 2026-09-24  
**Status:** In implementation. Phases 0, 1 and 2 complete. Phase 3: every implementation task 3.1-3.9 is now written, and 3.10 and 3.11 have passed against live AWS -- the 3.11 test on 2026-09-25 (build-20260925-1718) closed out `s3PutObject` and `setupDatabase` and proved the bucket CORS rule from a real browser origin. `exportAllTables` (3.7) was written on branch `feature/athena-connector-export` (build-20260925-1756) with new decisions D8, D9 and D10. Remaining in Phase 3: the two browser smoke tests, 3.13 for export and then 3.12 for import, run together as one round trip. Nothing in Phase 3 is UI-reachable yet; the screens are Phases 4-6.
...
const built = athenaBuildTableCSV('critical_data_set', getAppData());
...
const r = await AthenaConnector.exportAllTables(cfg, getAppData(), (s, t, m) => console.log(s + '/' + t, m));
...
`getAppData()` is shorthand for whatever the console exposes as the current `data` object -- read it from the React tree or from `localStorage.getItem('moj_dq_store_v1')`.
