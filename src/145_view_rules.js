...
function RulesRuleRow({ ruleEntry, critGroupsSorted, critLevelsById, dimensionsById,
  ...
  onOpenSql, onToggleAutomated, canEdit, accent }) {
    ...
    <div style={{ background:'var(--bg2)', border:'1px solid var(--border)',
      ...
      <div style={{ display:'flex', alignItems:'flex-start', gap:10, padding:'11px 14px',
        ...
        <div style={{ display:'flex', alignItems:'center', gap:6, flexShrink:0 }}>
          <button
            onClick={e => { e.stopPropagation(); onToggleAutomated(rule); }}
            disabled={!canEdit}
...
function RuleExplorerView() {
  ...
  const handleAddRule  = () => setRulePanel({ record: {
    ...
    source_code_link: null, automated: true, retiring_timestamp: null,
  }});
        ...
        <div style={{ display:'flex', flexDirection:'column', gap:10 }}>
          {filtered.map(ruleEntry => (
            <RulesRuleRow
              ...
              onOpenSql={handleOpenSql} onToggleAutomated={handleToggleAutomated} canEdit={canEdit} accent={accent}
            />
          ))}
        </div>
      )}
