...
function computeStewardGaps(data, stewardIdentity) {
  ...
  // All integrity issues (PK duplicates + FK violations across all tables)
  const { issues: allIssues } = runHealthCheck(data);

  // Scoped issues: build a comprehensive per-table PK scope set for this steward
  const myAllocationIds = new Set(
    live('data_quality_rule_allocation')
      .filter(function(a) { return myCdeIds.has(a.critical_data_element_id); })
      .map(function(a) { return a.data_quality_rule_allocation_id; })
  );
  const myRuleIds = new Set(
    live('data_quality_rule_allocation')
      .filter(function(a) { return myCdeIds.has(a.critical_data_element_id); })
      .map(function(a) { return a.data_quality_rule_id; })
  );
  const myCriticalityIds = new Set(
    live('cde_criticality')
      .filter(function(c) { return myCdeIds.has(c.critical_data_element_id); })
      .map(function(c) { return c.cde_criticality_id; })
  );
  const myStewardshipIds = new Set(
    live('stewardship')
      .filter(function(s) { return myCdsIds.has(s.critical_data_set_id); })
      .map(function(s) { return s.stewardship_id; })
  );
  const myPatronIds = new Set(
    live('data_patron')
      .filter(function(p) { return myAgencyIds.has(p.executive_agency_id); })
      .map(function(p) { return p.data_patron_id; })
  );
  const myOwnerIds = new Set(
    live('data_owner')
      .filter(function(o) { return myAgencyDirIds.has(o.directorate_id); })
      .map(function(o) { return o.data_owner_id; })
  );
  const myCgwIds = new Set(
    live('criticality_group_weight')
      .filter(function(w) { return myAgencyIds.has(w.executive_agency_id); })
      .map(function(w) { return w.criticality_group_weight_id; })
  );
  const myQdwIds = new Set(
    live('quality_dimension_weight')
      .filter(function(w) { return myAgencyIds.has(w.executive_agency_id); })
      .map(function(w) { return w.quality_dimension_weight_id; })
  );
  const myCdeDbTableKeys = new Set(
    myCdes.map(function(c) { return (c.source_database_name || '') + '|' + (c.source_table_name || ''); })
  );
  const myDdlIds = new Set(
    live('source_table_ddl')
      .filter(function(d) { return myCdeDbTableKeys.has((d.source_database_name || '') + '|' + (d.source_table_name || '')); })
      .map(function(d) { return d.source_table_ddl_id; })
  );
  const myCdeFieldKeys = new Set(
    myCdes.map(function(c) { return (c.source_database_name || '') + '|' + (c.source_table_name || '') + '|' + (c.source_field_name || ''); })
  );
  const myProfilingIds = new Set(
    live('field_profiling')
      .filter(function(f) { return myCdeFieldKeys.has((f.source_database_name || '') + '|' + (f.source_table_name || '') + '|' + (f.source_field_name || '')); })
      .map(function(f) { return f.field_profiling_id; })
  );
  const myShortlistGroupIds = new Set(
    (data.shortlist_group || [])
      .filter(function(g) { return myAgencyDirIds.has(g.directorate_id); })
      .map(function(g) { return g.shortlist_group_id; })
  );
  // Reference tables have no steward scope -- include all their PKs
  function allPks(t) { return new Set((data[t] || []).map(function(r) { return r[SCHEMA[t] && SCHEMA[t].pk]; })); }

  const scopePksByTable = {
    executive_agency_type:        allPks('executive_agency_type'),
    executive_agency:             myAgencyIds,
    directorate:                  myAgencyDirIds,
    critical_data_set:            myCdsIds,
    critical_data_element:        myCdeIds,
    data_quality_rule:            myRuleIds,
    data_quality_rule_allocation: myAllocationIds,
    cde_criticality:              myCriticalityIds,
    stewardship:                  myStewardshipIds,
    data_patron:                  myPatronIds,
    data_owner:                   myOwnerIds,
    data_steward:                 allPks('data_steward'),
    quality_dimension:            allPks('quality_dimension'),
    criticality_group:            allPks('criticality_group'),
    criticality_level:            allPks('criticality_level'),
    criticality_group_weight:     myCgwIds,
    quality_dimension_weight:     myQdwIds,
    steward_role_type:            allPks('steward_role_type'),
    source_table_ddl:             myDdlIds,
    field_profiling:              myProfilingIds,
    shortlist_group:              myShortlistGroupIds,
  };

  const scopedIssues = allIssues.filter(function(iss) {
    var scopeSet = scopePksByTable[iss.table];
    if (!scopeSet) return true;
    return scopeSet.has(iss.pk);
  });
  ...
  return {
    ...
    myCdsSummary, allIssues, scopedIssues,
  };
}
...
function DashboardScreen() {
  ...
  const cdsById = {};
  (data.critical_data_set || []).forEach(c => { cdsById[c.critical_data_set_id] = c; });
  ...
  const cards = [
    ...
    {
      count: gaps.incompleteCdes.length,
      label: 'Incomplete Definitions',
      description: 'CDEs missing a definition or explanation.',
      items: gaps.incompleteCdes.map(function(cde) {
        var cds = cdsById[cde.critical_data_set_id];
        return {
          label:      cde.data_element_name || ('CDE #' + cde.critical_data_element_id),
          sublabel:   cds ? cds.data_set_name : null,
          navigateTo: { screen: 'table', table: 'critical_data_element' },
        };
      }),
    },
    {
  ...
  return (
    <div className="fade-in">
      ...
      {/* Data integrity -- always shown when identity is set; master sees full DB, stewards see scoped */}
      <div style={{ marginBottom: 24 }}>
        <div
          ...
          style={{
            ...
            background: 'var(--bg2)',
            border: '1px solid ' + (displayIssues.length ? 'var(--amber)' : 'var(--border)'),
            borderLeft: '3px solid ' + (displayIssues.length ? 'var(--amber)' : 'var(--border)'),
            borderRadius: fkOpen ? 'var(--radius) var(--radius) 0 0' : 'var(--radius)',
          }}
        >
          <span style={{
            fontWeight: 600, fontSize: 12,
            color: displayIssues.length ? 'var(--amber)' : 'var(--text3)',
          }}>
            Data integrity
          </span>
          {isMaster && (
          ...
          <span style={{
            ...
            background: displayIssues.length ? 'var(--amber-bg)' : 'var(--bg3)',
            color: displayIssues.length ? 'var(--amber)' : 'var(--text3)',
            border: '1px solid ' + (displayIssues.length ? 'var(--amber)' : 'var(--border)'),
          }}>
